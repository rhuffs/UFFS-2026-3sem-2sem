...
void removeHeap(Heap *H){
    ...
    int i = 0;
    while(left(i) < H->size){
        int maiorFilho = left(i);
        int rightChild = right(i);

        if(rightChild < H->size && H->heap[rightChild] > H->heap[maiorFilho]){
            maiorFilho = rightChild;
        }

        if(H->heap[i] < H->heap[maiorFilho]){
            swap(&H->heap[i], &H->heap[maiorFilho]);
            i = maiorFilho;
        } else {
            break;
        }
